...
document.addEventListener('DOMContentLoaded', () => {
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', async (e) => {
            ...
            const ngayChieu = e.currentTarget.dataset.ngayChieu;

            // 1. Xác nhận trước khi xóa
            if (!confirm(`Bạn có chắc chắn muốn xóa lịch chiếu "${ShowtimeID}" (ID: ${ngayChieu})? Hành động này không thể hoàn tác.`)) {
                return; // Ngừng nếu người dùng hủy
            }

            try {
                // 2. Gửi yêu cầu DELETE đến server (endpoint: /movies/:id)
                const response = await fetch(`/employee/xoaLichChieu/${ShowtimeID}`, {
                    method: 'DELETE',
                    headers: {
                ...
                if (response.ok && result.success) { // Kiểm tra cả status HTTP và success flag
                    ...
                    const rowToRemove = document.getElementById(`movie-row-${ShowtimeID}`);
                    if (rowToRemove) {
